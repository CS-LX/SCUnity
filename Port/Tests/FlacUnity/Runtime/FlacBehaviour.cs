...
namespace SCUnity.Validation
{
    ...
    public sealed class FlacBehaviour : MonoBehaviour
    {
        [Serializable] public sealed class Check { public string name; public bool passed; }
        [Serializable] public sealed class Result
        {
            public string schema = "scunity-flac-v1";
        ...
        void Start()
        {
            ...
            try
            {
                ...
                int outputIndex = Array.IndexOf(args, "-flacOutput");
                if (outputIndex < 0) throw new InvalidOperationException("Missing -flacOutput.");
                foreach (var check in FlacValidation.Entry.Run(Path.Combine(Application.streamingAssetsPath, "Flac"), args[outputIndex + 1]))
            ...
            Debug.Log("SCUnity FLAC: " + (result.passed ? "PASS" : "FAIL") + " " + result.error);
