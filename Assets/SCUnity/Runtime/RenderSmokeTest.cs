...
namespace SCUnity.Runtime {
    internal sealed class RenderSmokeTest : IDisposable {
        ...
        public void Draw(SurvivalcraftGame game, string output) {
            ...
            target = new E.RenderTarget2D(
                64,
                64,
                1,
                E.ColorFormat.Rgba8888,
                E.DepthFormat.Depth24Stencil8
            );
            ...
            E.Display.Clear(Engine.Color.Black, 1, 0);
            primitives.TexturedBatch(
                target,
                false,
                0,
                E.DepthStencilState.None,
                E.RasterizerState.CullNone,
                E.BlendState.Opaque,
                E.SamplerState.PointClamp
            )
            .QueueQuad(
                new Engine.Vector2(0, 0),
                new Engine.Vector2(128, 128),
                0,
                Engine.Vector2.Zero,
                Engine.Vector2.One,
                Engine.Color.White
            ...
            primitives.FlatBatch(0, E.DepthStencilState.None, E.RasterizerState.CullNone, E.BlendState.NonPremultiplied).QueueQuad(new Engine.Vector2(240, 64), new Engine.Vector2(288, 128), 0, new Engine.Color(255, 0, 0, 128));
            primitives.Flush();
        ...
        void Quad(float x1, float y1, float x2, float y2, float depth, Engine.Color color) {
            primitives.FlatBatch(0, E.DepthStencilState.Default, E.RasterizerState.CullNone, E.BlendState.Opaque).QueueQuad(new Engine.Vector2(x1, y1), new Engine.Vector2(x2, y2), depth, color);
            primitives.Flush();
        }
        ...
        IEnumerator Capture(string output) {
            ...
            try {
                ...
                Check(
                    ...
                    80,
                    Color.red,
                    "Near geometry occludes later far geometry in render texture"
                );
                Check(
                    image,
                    96,
                    80,
                    Color.blue,
                    "Far geometry appears outside the near surface"
                );
                Check(
                    image,
                    32,
                    16,
                    Color.green,
                    "Render texture preserves vertical orientation"
                );
                Check(
                    image,
                    192,
                    80,
                    Color.red,
                    "Backbuffer uses the bound depth attachment"
                );
                Check(
                    image,
                    256,
                    32,
                    Color.blue,
                    "Backbuffer depth clear and range are correct"
                );
                Check(
                    image,
                    256,
                    96,
                    new Color(.5f, 0, .5f),
                    "Nonpremultiplied alpha blends correctly"
                );
            }
        ...
        void Check(Texture2D image, int x, int y, Color expected, string check) {
            ...
            if (Mathf.Abs(actual.r - expected.r) > .03f
                || Mathf.Abs(actual.g - expected.g) > .03f
                || Mathf.Abs(actual.b - expected.b) > .03f)
