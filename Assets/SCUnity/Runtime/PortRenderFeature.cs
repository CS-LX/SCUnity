    ...
    public sealed class PortRenderFeature : ScriptableRendererFeature {
        sealed class Pass : ScriptableRenderPass {
            ...
            public override void RecordRenderGraph(RenderGraph graph, ContextContainer frameData) {
                if (PortRenderer.Active == null) return;
                using (var builder = graph.AddUnsafePass<Data>("Survivalcraft ordered draws", out var data)) {
                    data.Color = frameData.Get<UniversalResourceData>().activeColorTexture;
                    data.Depth = frameData.Get<UniversalResourceData>().activeDepthTexture;
                    data.Renderer = PortRenderer.Active;
                    ...
                    builder.SetRenderFunc((Data d, UnsafeGraphContext context) => {
                            ...
                            d.Renderer.Render(cmd, d.Color, d.Depth);
