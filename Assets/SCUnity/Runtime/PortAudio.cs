...
namespace SCUnity.Runtime
{
    ...
    public sealed class PortAudio : MonoBehaviour
    {
        ...
        public void Initialize()
        {
            ...
            output.loop = true;
            output.priority = 0;
            clockClip = AudioClip.Create("Survivalcraft PCM clock", rate, 2, rate, false);
            output.clip = clockClip;
            running = true;
            output.Play();
            AudioSettings.OnAudioConfigurationChanged += ConfigurationChanged;
        }
        void ConfigurationChanged(bool deviceChanged) { rate = AudioSettings.outputSampleRate; }
        void OnAudioFilterRead(float[] samples, int channels)
        {
            if (!running) { Array.Clear(samples, 0, samples.Length); return; }
