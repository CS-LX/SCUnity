...
namespace SCUnity.Runtime
{
    ...
    internal sealed class WorldSmokeTest : IDisposable
    {
        ...
        public WorldSmokeTest(SurvivalcraftGame game, string output)
        {
            ...
            observer.Patch(AccessTools.Method(typeof(GameManager), nameof(GameManager.LoadProject)),
                postfix: new HarmonyMethod(typeof(WorldSmokeTest), nameof(AddTestPlayer)));
            foreach (var type in typeof(Game.Program).Assembly.GetTypes())
                if (!type.IsAbstract && typeof(IDrawable).IsAssignableFrom(type))
            ...
            world = WorldsManager.CreateWorld(new WorldSettings {
                Name = "SCUnity world validation", CustomWorldSeed = true, WorldSeed = 123456, Seed = "123456",
                OriginalSerializationVersion = VersionsManager.SerializationVersion,
                GameMode = GameMode.Creative, TerrainGenerationMode = TerrainGenerationMode.FlatContinent,
        ...
        static void AddTestPlayer()
        {
            ...
            if (players.PlayersData.Count == 0)
                players.AddPlayerData(new PlayerData(GameManager.Project) {
                    Name = "UnityPlayer", InputDevice = WidgetInputDevice.None,
        ...
        public void Tick()
        {
            ...
            var players = GameManager.Project.FindSubsystem<SubsystemPlayers>(true);
            if (players.PlayersData.Count == 0 || !players.PlayersData[0].IsReadyForPlaying) return;
            ...
            if (phase == 0 && playingFrames >= 30 && UnityEngine.Time.realtimeSinceStartup - playingSince >= 3)
            {
            ...
            else if (phase == 1 && captured)
            {
                ...
                using (var stream = File.Create(Path.Combine(output, "world.scworld")))
                    WorldsManager.ExportWorld(world.DirectoryName, stream);
                ...
                phase = 2; playingFrames = 0;
            }
            else if (phase == 2 && playingFrames >= 30 && UnityEngine.Time.realtimeSinceStartup - playingSince >= 3)
            {
                ...
                Require(GameManager.Project.FindSubsystem<SubsystemTerrain>(true).Terrain.GetCellValue(0, 70, 0) == 3,
                    "Placed block survived project disposal and reload");
        ...
        public void Dispose() { observer.UnpatchSelf(); rendering.Dispose(); }
    }
}
