...
本阶段可验收“加载 → 主菜单 → 鼠标进入设置 → Escape 返回”，声音的 12 项输出/状态检查已通过；世界渲染、IME 候选窗、原生文件对话框、窗口模式和重启仍待补齐。
不要将能进入菜单理解为完整游戏或完整模组兼容已经验收。

存档目录为 `Application.persistentDataPath/Survivalcraft`，启动时会打印实际路径，不指向旧游戏目录。
自动测试仅在命令行显式传入 `-scunity-smoke-output <独立目录>` 时启用；附加 `-scunity-audio-test` 验证 Unity Listener 的真实输出。声音证据见 `Port/Tests/Baselines/desktop-audio`。
