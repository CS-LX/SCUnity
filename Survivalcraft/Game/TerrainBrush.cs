...
namespace Game {
    public class TerrainBrush {
        public struct Cell : IComparable<Cell> {
            ...
            public int CompareTo(Cell other) => Key(X, Y, Z) - Key(other.X, other.Y, other.Z);
        }
        ...
        public class Brush {
            ...
            public static implicit operator Brush(int value) => new() { m_value = value };

            public static implicit operator Brush(Func<int?, int?> handler) => new() { m_handler1 = handler };

            public static implicit operator Brush(Func<Point3, int?> handler) => new() { m_handler2 = handler };
        ...
        public class Counter {
            ...
            public static implicit operator Counter(int value) => new() { m_value = value };

            public static implicit operator Counter(Func<int?, int> handler) => new() { m_handler1 = handler };

            public static implicit operator Counter(Func<Point3, int> handler) => new() { m_handler2 = handler };
        ...
        public static int Key(int x, int y, int z) => y + 128 + ((x + 128) << 8) + ((z + 128) << 16);
        ...
        public int CountNonDiagonalNeighbors(int x, int y, int z, Counter counter) => 0
        ...
        public int? GetValue(Point3 p) => GetValue(p.X, p.Y, p.Z);
        ...
        public void AddCell(int x, int y, int z, Brush brush) {
            ...
            if (num.HasValue) {
                int key = Key(x, y, z);
                m_cellsDictionary[key] = new Cell { X = (sbyte)x, Y = (sbyte)y, Z = (sbyte)z, Value = num.Value };
                m_cells = null;
            }
        }
        ...
        public void PaintFastSelective(Terrain terrain,
            int x,
            int y,
            int z,
            int minX,
            ...
            for (int i = 0; i < cells.Length; i++) {
                ...
                if (num >= 0
                    ...
                    if (chunk.GetCellContentsFast(num, num2, num3) != 18
                        && terrain.GetCellContents(num4 - 1, y2, num5) != 18
                        && terrain.GetCellContents(num4 + 1, y2, num5) != 18
                        && terrain.GetCellContents(num4, y2, num5 - 1) != 18
                        && terrain.GetCellContents(num4, y2, num5 + 1) != 18
        ...
        public void PaintFastAvoidWater(Terrain terrain,
            int x,
            int y,
            int z,
            int minX,
        ...
        public void PaintFast(Terrain terrain,
            int x,
            int y,
            int z,
            int minX,
