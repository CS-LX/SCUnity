...
namespace Game {
    public class TerrainContentsGenerator21 : ITerrainContentsGenerator {
        public class Random {
            ...
            public int UniformInt(int min, int max) => (int)(min + Int() * (long)(max - min + 1) / 2147483648L);
        ...
        public class OldRandom {
            public class InternalRandom {
                ...
                public double Sample() => InternalSample() * 4.6566128752457969E-10;
            }
            ...
            public OldRandom() => m_random = new InternalRandom(997 * m_seed++);

            public OldRandom(int seed) => m_random = new InternalRandom(seed);
            ...
            public bool Bool() => m_random.Next() % 2 == 0;

            public int UniformInt(int min, int max) => m_random.Next(min, max + 1);

            public float UniformFloat(float min, float max) => (float)m_random.NextDouble() * (max - min) + min;
            ...
            public Vector2 Vector2(float length) => Engine.Vector2.Normalize(new Vector2(UniformFloat(-1f, 1f), UniformFloat(-1f, 1f))) * length;

            public Vector2 UniformVector2(float minLength, float maxLength) =>
                Engine.Vector2.Normalize(new Vector2(UniformFloat(-1f, 1f), UniformFloat(-1f, 1f))) * UniformFloat(minLength, maxLength);

            public Vector3 Vector3(float length) =>
                Engine.Vector3.Normalize(new Vector3(UniformFloat(-1f, 1f), UniformFloat(-1f, 1f), UniformFloat(-1f, 1f))) * length;

            public Vector3 UniformVector3(float minLength, float maxLength) =>
                Engine.Vector3.Normalize(new Vector3(UniformFloat(-1f, 1f), UniformFloat(-1f, 1f), UniformFloat(-1f, 1f)))
        ...
        public class Grid2d {
            ...
            public float Get(int x, int y) => m_data[x + y * m_sizeX];
        ...
        public class Grid3d {
            ...
            public float Get(int x, int y, int z) => m_data[x + y * m_sizeX + z * m_sizeXY];
        ...
        public TerrainContentsGenerator21(SubsystemTerrain subsystemTerrain) {
            ...
            m_subsystemBottomSuckerBlockBehavior = subsystemTerrain.Project.FindSubsystem<SubsystemBottomSuckerBlockBehavior>(true);
            ...
            OldRandom oldRandom = new(100 + m_seed);
        ...
        public float CalculateMountainRangeFactor(float x, float z) => 1f
            - MathF.Abs(
                2f * SimplexNoise.OctavedNoise(x + m_mountainsOffset.X, z + m_mountainsOffset.Y, TGMountainsPeriod / TGBiomeScaling, 3, 1.91f, 0.75f)
                - 1f
            );
        ...
        public float CalculateHeight(float x, float z) {
            ...
            float num13 = MathUtils.Saturate(
                1.5f - num12 * MathF.Abs(2f * SimplexNoise.OctavedNoise(x + m_riversOffset.X, z + m_riversOffset.Y, 0.001f, 4, 2f, 0.5f) - 1f)
            );
        ...
        public int CalculateTemperature(float x, float z) {
            if (TGNewBiomeNoise) {
                return Math.Clamp(
                    (int)(MathUtils.Saturate(
                            4f
                            * SimplexNoise.OctavedNoise(x + m_temperatureOffset.X, z + m_temperatureOffset.Y, 0.0015f / TGBiomeScaling, 5, 2f, 0.7f)
                            - 1.6f
                            + m_worldSettings.TemperatureOffset / 16f
                        )
                        * 16f),
                    0,
                    15
            ...
            return Math.Clamp(
                (int)((MathUtils.Saturate(
                            4f
                            * SimplexNoise.OctavedNoise(x + m_temperatureOffset.X, z + m_temperatureOffset.Y, 0.0006f / TGBiomeScaling, 4, 1.93f, 1f)
                            - 1.6f
                        )
                        + m_worldSettings.TemperatureOffset / 16f)
                    * 16f),
        ...
        public int CalculateHumidity(float x, float z) {
            if (TGNewBiomeNoise) {
                return Math.Clamp(
                    (int)(MathUtils.Saturate(
                            4f * SimplexNoise.OctavedNoise(x + m_humidityOffset.X, z + m_humidityOffset.Y, 0.0012f / TGBiomeScaling, 5, 2f, 0.7f)
                            - 1.2f
                            + m_worldSettings.HumidityOffset / 16f
                        )
                        * 16f),
                    0,
                    15
            ...
            return Math.Clamp(
                (int)((MathUtils.Saturate(
                            4f * SimplexNoise.OctavedNoise(x + m_humidityOffset.X, z + m_humidityOffset.Y, 0.0008f / TGBiomeScaling, 5, 1.97f, 1f)
                            - 1.5f
                        )
                        + m_worldSettings.HumidityOffset / 16f)
                    * 16f),
        ...
        public float CalculateOceanShoreX(float z) => m_oceanCorner.X
            + TGShoreFluctuations * SimplexNoise.OctavedNoise(z, 0f, 0.005f / TGShoreFluctuationsScaling, 4, 1.95f, 1f);

        public float CalculateOceanShoreZ(float x) => m_oceanCorner.Y
            + TGShoreFluctuations * SimplexNoise.OctavedNoise(0f, x, 0.005f / TGShoreFluctuationsScaling, 4, 1.95f, 1f);
                ...
                for (int j = -1; j <= 1; j++) {
                    ...
                    if (random.Bool(0.02f + 0.01f * num4)) {
                        ...
                        for (int num14 = 0; num14 < num13; num14++) {
                            ...
                            m_waterPocketBrushes[random.UniformInt(0, m_waterPocketBrushes.Count - 1)]
                    ...
                    if (random.Bool(0.06f + 0.05f * num4)) {
                        ...
                        for (int num22 = 0; num22 < num21; num22++) {
                            ...
                            m_magmaPocketBrushes[random.UniformInt(0, m_magmaPocketBrushes.Count - 1)]
        ...
        public void GenerateCaves(TerrainChunk chunk) {
            ...
            for (int i = x - 2; i <= x + 2; i++) {
                for (int j = y - 2; j <= y + 2; j++) {
                    ...
                    if (direction.Y > -0.6f) {
                        list.Add(new CavePoint { Position = position, Direction = direction, BrushType = 0, Length = random.UniformInt(80, 240) });
                    }
                    int num6 = i * 16 + 8;
                    int num7 = j * 16 + 8;
                    int num8 = 0;
                    while (num8 < list.Count) {
                        ...
                        list2[random.UniformInt(0, list2.Count - 1)]
                            .PaintFastAvoidWater(
                                chunk,
                                Terrain.ToCell(cavePoint.Position.X),
                                Terrain.ToCell(cavePoint.Position.Y),
                        ...
                        if (random.Bool(0.5f)) {
                            Vector3 v3 = Vector3.Normalize(random.Vector3(1f, true));
                            if ((num9 < -25.5f && v3.X < 0f)
                                || (num9 > 25.5f && v3.X > 0f)) {
                                v3.X = 0f - v3.X;
                            }
                            if ((num10 < -25.5f && v3.Z < 0f)
                                || (num10 > 25.5f && v3.Z > 0f)) {
                                v3.Z = 0f - v3.Z;
                            }
                            if ((cavePoint.Direction.Y < -0.5f && v3.Y < -10f)
                                || (cavePoint.Direction.Y > 0.1f && v3.Y > 0f)) {
                                v3.Y = 0f - v3.Y;
                            }
                            cavePoint.Direction = Vector3.Normalize(cavePoint.Direction + 0.5f * v3);
                        }
                        if (cavePoint.StepsTaken > 20
                            && random.Bool(0.06f)) {
        ...
        public void GenerateTreesAndLogs(TerrainChunk chunk) {
            ...
            for (int i = x2; i <= x2; i++) {
                for (int j = y2; j <= y2; j++) {
                    Game.Random random = new(m_seed + i + 3943 * j);
                    int humidity = CalculateHumidity(i * 16, j * 16);
                    int num3 = CalculateTemperature(i * 16, j * 16);
                    float num4 = MathUtils.Saturate(
                        (SimplexNoise.OctavedNoise(i, j, 0.1f, 2, 2f, 0.5f) - 0.25f) / 0.2f + (random.Bool(0.25f) ? 0.5f : 0f)
                    );
                    int num5 = 0;
                    if (num4 > 0.95f) {
                    ...
                    for (int k = 0; k < 8; k++) {
                        ...
                        TreeType? treeType = PlantsManager.GenerateRandomTreeType(
                            random,
                            num3 + SubsystemWeather.GetTemperatureAdjustmentAtHeight(num9),
                            humidity,
                            num9
                        );
                        if (treeType.HasValue) {
                            int treeTrunkValue = PlantsManager.GetTreeTrunkValue(treeType.Value);
                            treeTrunkValue = Terrain.ReplaceData(
                                treeTrunkValue,
                            ...
                            for (int m = 0; m < num10; m++) {
                                ...
                                if (m > num10 / 2) {
                                    ...
                                    if (random.Bool(0.3f)
                                        && !BlocksManager.Blocks[Terrain.ExtractContents(value3)].IsCollidable_(value3)) {
                                        terrain.SetCellValueFast(num13 + point2.X, num9, num14 + point2.Z, treeLeavesValue);
                                    }
                                    if (random.Bool(0.05f)
                                        && !BlocksManager.Blocks[Terrain.ExtractContents(value4)].IsCollidable_(value4)) {
                                        terrain.SetCellValueFast(num13 + point2.X, num9, num14 + point2.Z, treeTrunkValue);
                                    }
                                    if (random.Bool(0.3f)
                                        && !BlocksManager.Blocks[Terrain.ExtractContents(value5)].IsCollidable_(value5)) {
                                        terrain.SetCellValueFast(num13 - point2.X, num9, num14 - point2.Z, treeLeavesValue);
                                    }
                                    if (random.Bool(0.05f)
                                        && !BlocksManager.Blocks[Terrain.ExtractContents(value6)].IsCollidable_(value6)) {
                                        terrain.SetCellValueFast(num13 - point2.X, num9, num14 - point2.Z, treeTrunkValue);
                                    }
                                    if (random.Bool(0.1f)
                                        && !BlocksManager.Blocks[Terrain.ExtractContents(value7)].IsCollidable_(value7)) {
                    ...
                    for (int n = 0; n < 32; n++) {
                        ...
                        if (!BlocksManager.Blocks[Terrain.ExtractContents(value1)].IsCollidable_(value1)
                            && !BlocksManager.Blocks[Terrain.ExtractContents(value2)].IsCollidable_(value2)
                            && !BlocksManager.Blocks[Terrain.ExtractContents(value3)].IsCollidable_(value3)
                            && !BlocksManager.Blocks[Terrain.ExtractContents(value4)].IsCollidable_(value4)) {
                            TreeType? treeType2 = PlantsManager.GenerateRandomTreeType(
                                random,
                                num3 + SubsystemWeather.GetTemperatureAdjustmentAtHeight(num19),
                                humidity,
                                num19
                            );
                    ...
                    switch (Terrain.ExtractContents(chunk.GetCellValueFast(num5, num6, num7))) {
                        ...
                        case 67: {
                            ...
                            for (int k = 0; k < 4; k++) {
                                ...
                                int num9 = num6 - 1;
                                while (num9 >= 1
                                    && chunk.GetCellContentsFast(num5 + point.X, num9, num7 + point.Z) == 0
        ...
        public void GenerateTraps(TerrainChunk chunk) {
            ...
            if (!random.Bool(0.15f)
                || !(CalculateOceanShoreDistance(chunk.Origin.X, chunk.Origin.Y) > 50f)) {
                return;
            }
                ...
                while (true) {
                    if (num8 < num4 + num2 + 1) {
                        for (int j = num5 - 1; j < num5 + num3 + 1; j++) {
                            ...
                            while (num9 >= num10
                                && chunk.GetCellContentsFast(num8, num9, j) != 8) {
                                num9--;
                            }
                            if (num7.HasValue
                                && num7 != num9) {
                    ...
                    for (int k = 0; k < 4; k++) {
                        ...
                        for (int num22 = num19; num22 <= num20; num22++) {
                            ...
                            if (num17 < 0.3f) {
                                ...
                                int value2 = Terrain.MakeBlockValue(
                                    contents,
                                    0,
                                ...
                                if (num22 == -1) {
                                    int value3 = Terrain.MakeBlockValue(
                                        contents,
                                        0,
            ...
            for (int i = 0; i < 16; i++) {
                for (int j = 0; j < 16; j++) {
                    ...
                    while (num3 >= 0) {
                        ...
                        if (num4 == 0
                            && num2 != 0
                            && BlocksManager.FluidBlocks[num2] != null) {
            ...
            for (int i = 0; i < 16; i++) {
                for (int j = 0; j < 16; j++) {
                    ...
                    while (num3 >= 0) {
                        ...
                        int num4 = Terrain.ExtractContents(cellValueFast);
                        if (num4 != num2
                            && BlocksManager.FluidBlocks[num4] != null
                            && BlocksManager.FluidBlocks[num2] == null) {
        ...
        public static void CreateBrushes() {
            ...
            for (int i = 0; i < 16; i++) {
                ...
                for (int j = 0; j < num; j++) {
                    Vector3 vector = 0.5f
                        * Vector3.Normalize(new Vector3(random.UniformFloat(-1f, 1f), random.UniformFloat(-1f, 1f), random.UniformFloat(-1f, 1f)));
                    int num2 = random.UniformInt(3, 8);
                    Vector3 zero = Vector3.Zero;
                    for (int k = 0; k < num2; k++) {
                        terrainBrush.AddBox(
                            (int)MathF.Floor(zero.X),
                            (int)MathF.Floor(zero.Y),
                            (int)MathF.Floor(zero.Z),
                            1,
                            1,
                            1,
                            16
            ...
            for (int l = 0; l < 16; l++) {
                ...
                for (int m = 0; m < num3; m++) {
                    Vector3 vector2 = 0.5f
                        * Vector3.Normalize(new Vector3(random.UniformFloat(-1f, 1f), random.UniformFloat(-1f, 1f), random.UniformFloat(-1f, 1f)));
                    int num4 = random.UniformInt(3, 6);
                    Vector3 zero2 = Vector3.Zero;
                    for (int n = 0; n < num4; n++) {
                        terrainBrush2.AddBox(
                            (int)MathF.Floor(zero2.X),
                            (int)MathF.Floor(zero2.Y),
                            (int)MathF.Floor(zero2.Z),
                            1,
                            1,
                            1,
                            39
            ...
            for (int num5 = 0; num5 < 16; num5++) {
                ...
                for (int num7 = 0; num7 < num6; num7++) {
                    Vector3 vector3 = 0.5f
                        * Vector3.Normalize(new Vector3(random.UniformFloat(-1f, 1f), random.UniformFloat(-2f, 2f), random.UniformFloat(-1f, 1f)));
                    int num8 = random.UniformInt(3, 6);
                    Vector3 zero3 = Vector3.Zero;
                    for (int num9 = 0; num9 < num8; num9++) {
                        terrainBrush3.AddBox(
                            (int)MathF.Floor(zero3.X),
                            (int)MathF.Floor(zero3.Y),
                            (int)MathF.Floor(zero3.Z),
                            1,
                            1,
                            1,
                            41
            ...
            for (int num10 = 0; num10 < 16; num10++) {
                ...
                for (int num12 = 0; num12 < num11; num12++) {
                    Vector3 vector4 = 0.5f
                        * Vector3.Normalize(
                            new Vector3(random.UniformFloat(-1f, 1f), random.UniformFloat(-0.25f, 0.25f), random.UniformFloat(-1f, 1f))
                        );
                    int num13 = random.UniformInt(4, 8);
                    Vector3 zero4 = Vector3.Zero;
                    for (int num14 = 0; num14 < num13; num14++) {
                        terrainBrush4.AddBox(
                            (int)MathF.Floor(zero4.X),
                            (int)MathF.Floor(zero4.Y),
                            (int)MathF.Floor(zero4.Z),
                            1,
                            1,
                            1,
                            100
            ...
            for (int num15 = 0; num15 < 16; num15++) {
                ...
                for (int num17 = 0; num17 < num16; num17++) {
                    Vector3 vector5 = 0.5f
                        * Vector3.Normalize(new Vector3(random.UniformFloat(-1f, 1f), random.UniformFloat(-1f, 1f), random.UniformFloat(-1f, 1f)));
                    int num18 = random.UniformInt(3, 6);
                    Vector3 zero5 = Vector3.Zero;
                    for (int num19 = 0; num19 < num18; num19++) {
                        terrainBrush5.AddBox(
                            (int)MathF.Floor(zero5.X),
                            (int)MathF.Floor(zero5.Y),
                            (int)MathF.Floor(zero5.Z),
                            1,
                            1,
                            1,
                            101
            ...
            for (int num20 = 0; num20 < 16; num20++) {
                ...
                for (int num22 = 0; num22 < num21; num22++) {
                    Vector3 vector6 = 0.5f
                        * Vector3.Normalize(new Vector3(random.UniformFloat(-1f, 1f), random.UniformFloat(-1f, 1f), random.UniformFloat(-1f, 1f)));
                    int num23 = random.UniformInt(3, 6);
                    Vector3 zero6 = Vector3.Zero;
                    for (int num24 = 0; num24 < num23; num24++) {
                        terrainBrush6.AddBox(
                            (int)MathF.Floor(zero6.X),
                            (int)MathF.Floor(zero6.Y),
                            (int)MathF.Floor(zero6.Z),
                            1,
                            1,
                            1,
                            112
            ...
            for (int num25 = 0; num25 < 16; num25++) {
                ...
                for (int num27 = 0; num27 < num26; num27++) {
                    Vector3 vector7 = 0.5f
                        * Vector3.Normalize(new Vector3(random.UniformFloat(-1f, 1f), random.UniformFloat(-1f, 1f), random.UniformFloat(-1f, 1f)));
                    int num28 = random.UniformInt(3, 6);
                    Vector3 zero7 = Vector3.Zero;
                    for (int num29 = 0; num29 < num28; num29++) {
                        terrainBrush7.AddBox(
                            (int)MathF.Floor(zero7.X),
                            (int)MathF.Floor(zero7.Y),
                            (int)MathF.Floor(zero7.Z),
                            1,
                            1,
                            1,
                            148
            ...
            for (int num30 = 0; num30 < 16; num30++) {
                ...
                for (int num32 = 0; num32 < num31; num32++) {
                    Vector3 vector8 = 0.5f
                        * Vector3.Normalize(
                            new Vector3(random.UniformFloat(-1f, 1f), random.UniformFloat(-0.75f, 0.75f), random.UniformFloat(-1f, 1f))
                        );
                    int num33 = random.UniformInt(6, 12);
                    Vector3 zero8 = Vector3.Zero;
                    for (int num34 = 0; num34 < num33; num34++) {
                        terrainBrush8.AddBox(
                            (int)MathF.Floor(zero8.X),
                            (int)MathF.Floor(zero8.Y),
                            (int)MathF.Floor(zero8.Z),
                            1,
                            1,
                            1,
                            2
            ...
            for (int num35 = 0; num35 < 16; num35++) {
                ...
                for (int num37 = 0; num37 < num36; num37++) {
                    ...
                    for (int num39 = 0; num39 < num38; num39++) {
                        terrainBrush9.AddBox(
                            (int)MathF.Floor(zero9.X),
                            (int)MathF.Floor(zero9.Y),
                            (int)MathF.Floor(zero9.Z),
                            1,
                            1,
                            1,
                            6
            ...
            for (int num40 = 0; num40 < 16; num40++) {
                ...
                for (int num42 = 0; num42 < num41; num42++) {
                    ...
                    for (int num44 = 0; num44 < num43; num44++) {
                        terrainBrush10.AddBox(
                            (int)MathF.Floor(zero10.X),
                            (int)MathF.Floor(zero10.Y),
                            (int)MathF.Floor(zero10.Z),
                            1,
                            1,
                            1,
                            66
            ...
            for (int num45 = 0; num45 < 16; num45++) {
                ...
                for (int num47 = 0; num47 < num46; num47++) {
                    Vector3 vector11 = 0.5f
                        * Vector3.Normalize(
                            new Vector3(random.UniformFloat(-1f, 1f), random.UniformFloat(-0.1f, 0.1f), random.UniformFloat(-1f, 1f))
                        );
                    int num48 = random.UniformInt(6, 12);
                    Vector3 zero11 = Vector3.Zero;
                    for (int num49 = 0; num49 < num48; num49++) {
                        terrainBrush11.AddBox(
                            (int)MathF.Floor(zero11.X),
                            (int)MathF.Floor(zero11.Y),
                            (int)MathF.Floor(zero11.Z),
                            1,
                            1,
                            1,
                            72
            ...
            for (int num50 = 0; num50 < 16; num50++) {
                ...
                for (int num52 = 0; num52 < num51; num52++) {
                    ...
                    for (int num54 = 0; num54 < num53; num54++) {
                        terrainBrush12.AddBox(
                            (int)MathF.Floor(zero12.X),
                            (int)MathF.Floor(zero12.Y),
                            (int)MathF.Floor(zero12.Z),
                            1,
                            1,
                            1,
                            7
            ...
            for (int num55 = 0; num55 < 16; num55++) {
                ...
                for (int num57 = 0; num57 < num56; num57++) {
                    ...
                    for (int num59 = 0; num59 < num58; num59++) {
                        terrainBrush13.AddBox(
                            (int)MathF.Floor(zero13.X),
                            (int)MathF.Floor(zero13.Y),
                            (int)MathF.Floor(zero13.Z),
                            1,
                            1,
                            1,
                            67
            ...
            for (int num60 = 0; num60 < 16; num60++) {
                ...
                for (int num62 = 0; num62 < num61; num62++) {
                    Vector3 vector14 = 0.5f
                        * Vector3.Normalize(new Vector3(random.UniformFloat(-1f, 1f), random.UniformFloat(-1f, 1f), random.UniformFloat(-1f, 1f)));
                    int num63 = random.UniformInt(5, 10);
                    Vector3 zero14 = Vector3.Zero;
                    for (int num64 = 0; num64 < num63; num64++) {
                        terrainBrush14.AddBox(
                            (int)MathF.Floor(zero14.X),
                            (int)MathF.Floor(zero14.Y),
                            (int)MathF.Floor(zero14.Z),
                            1,
                            1,
                            1,
                            3
            ...
            for (int num65 = 0; num65 < 4 * array.Length; num65++) {
                ...
                for (int num70 = 0; num70 < num69; num70++) {
                    ...
                    terrainBrush15.AddBox(
                        (int)MathF.Floor(vector15.X),
                        0,
                        (int)MathF.Floor(vector15.Y),
                        num72,
                        sizeY,
                        num72,
                        0
            ...
            for (int num73 = 0; num73 < 4 * array2.Length; num73++) {
                ...
                for (int num78 = 0; num78 < num77; num78++) {
                    ...
                    terrainBrush16.AddBox(
                        (int)MathF.Floor(vector16.X),
                        0,
                        (int)MathF.Floor(vector16.Y),
                        num80,
                        sizeY2,
                        num80,
                        0
            ...
            for (int num82 = 0; num82 < 7; num82++) {
                ...
                for (int num83 = 0; num83 < 3; num83++) {
                    ...
                    for (int num85 = 0; num85 < num84; num85++) {
                        ...
                        Vector3 vector17 = 0.5f
                            * new Vector3(random.UniformFloat(-1f, 1f), random.UniformFloat(0f, 1f), random.UniformFloat(-1f, 1f));
                        Vector3 zero15 = Vector3.Zero;
                        for (int num88 = 0; num88 < num87; num88++) {
                            terrainBrush17.AddBox(
                                (int)MathF.Floor(zero15.X) - num86 / 2,
                                (int)MathF.Floor(zero15.Y) - num86 / 2,
                                (int)MathF.Floor(zero15.Z) - num86 / 2,
                                num86,
                                num86,
                                num86,
                                0
