...
namespace System.Collections.Generic
{
    ...
    public class PriorityQueue<TElement, TPriority>
    {
        ...
        public PriorityQueue(IEnumerable<(TElement Element, TPriority Priority)> items, IComparer<TPriority>? comparer)
        {
            ...
            _nodes = QueueHelpers.ToArray(items, out _size);
            _comparer = InitializeComparer(comparer);
        ...
        /// </remarks>
        public UnorderedItemsCollection UnorderedItems => m_unity_UnorderedItems ??= new UnorderedItemsCollection(this);
        /// <summary>
        ...
        {
            if (elements is null) throw new ArgumentNullException("elements");
        ...
        public sealed class UnorderedItemsCollection : IReadOnlyCollection<(TElement Element, TPriority Priority)>, ICollection
        {
            ...
            IEnumerator<(TElement Element, TPriority Priority)> IEnumerable<(TElement Element, TPriority Priority)>.GetEnumerator() =>
                _queue.Count == 0 ? QueueHelpers.GetEmptyEnumerator<(TElement Element, TPriority Priority)>() :
                GetEnumerator();

            IEnumerator IEnumerable.GetEnumerator() => ((IEnumerable<(TElement Element, TPriority Priority)>)this).GetEnumerator();
        }

        private UnorderedItemsCollection? m_unity_UnorderedItems;    }
}
namespace System.Collections.Generic {internal static class QueueHelpers {public static T[] ToArray<T>(IEnumerable<T> items,out int count){var a=System.Linq.Enumerable.ToArray(items);count=a.Length;return a;}public static IEnumerator<T> GetEmptyEnumerator<T>()=>((IEnumerable<T>)Array.Empty<T>()).GetEnumerator();}}
